...
def validate_product_order_items(order_data, response_data):
    ...
    for request_item, response_item in zip(request_product_order_items, response_product_order_items):
      excluded_properties = ["note", "product", "relatedContactInformation", "requestedCompletionDate","state","id","action","coordinatedActionDelay"]
      response_excluded_properties = ["charge", "completionDate", "expectedCompletionDate", "expediteAcceptedIndicator", "itemTerm", "milestone", "state", "stateChange", "terminationError","previoustate","id","action"]
