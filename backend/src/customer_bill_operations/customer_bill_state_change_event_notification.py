...
def customer_bill_state_change_event_notification(info):
    ...
    try:
        ...
        if info.eventType != "customerBillStateChangeEvent":
            status_msg_code = 422
            message = "The eventType must be 'customerBillStateChangeEvent'"
        ...
        if info.event.id not in list_of_keys :
            ...
            current_state = customer_bill_data.get("state")
            previous_state = customer_bill_data.get("previousState")
            
            if current_state != previous_state:
                customer_bill_data["previousState"] = current_state
                create_response_json(info.event.id, customer_bill_data, file_name)
                ...
                reason = "No change in CustomerBill 'state'."
                reference_error = None
