...
def retrive_catalog_subscription(id, buyerId, sellerId):
    ...
    try:
        ...
        response_file = "catalog_events_subscription.json"
        file_name = current_directory / 'responses'/response_file

        if not file_name.exists():
            return raise_exception(status_msg_code=404, message="File not found", reason="File not found", reference_error=None, message_code="notFound", property_path=None)
        try:
