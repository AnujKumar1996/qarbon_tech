...
from src.field_mapping.map_order_list_fields import map_cc_list_fields
...
def create_a_product_order(header_request:Request,
    ...
    if action == "add":
        try:
            ...
            response = create_product_order(order_dict, buyerId, sellerId, ccLoaAttachmentId, header_token,refresh_token)
            return response
        ...
        try:
            order_dict = order.model_dump(by_alias=True)
            response = disconnect_product_order(order_dict, action, buyerId, sellerId,ccLoaAttachmentId,header_token, refresh_token)
            return response
...
def retrieves_a_productorder_by_id(header_request: Request,
    response: Response,
    id: str,
    buyerId: str = Query(
    description= '''An identifier indicating the source(north) from which the transaction originated.
    ...
    qcl_response = call_qcl_order_details(mapped_data, header_token, refresh_token)
...
def lists_or_finds_productorder_objects(header_request: Request,
    response: Response,
    ...
    try:
        ...
        is_mapped, msg_statuscode, mapped_data, reason, reference_error, message_code, property_path = map_cc_list_fields(buyerId, sellerId, offset, limit, orderDate_gt, orderDate_lt)
        if not is_mapped and isinstance(mapped_data, str):
            return raise_exception(msg_statuscode, mapped_data, reason, reference_error, message_code, property_path)
        
        qcl_response = call_qcl_order_list(mapped_data, header_token, refresh_token)
        
        if isinstance(qcl_response, str):
            return raise_exception(404, "sonata_payloads.json or properties.json file not found", "Not found", None, "notFound", None)

        
        if qcl_response.status_code not in (200, 201): 
        ...
        if qcl_response.status_code == 422:
            ...
            for _, order_info in data.items():
                ...
                if (
                    ...
                    (externalId == "" or order_info.get("externalId") == externalId) and
                    (projectId == "" or order_info.get("projectId") == projectId) and
...
@router.patch('/v1/MEF/lsoSonata/productOrderingManagement/productOrder/{id}', tags=["productOrder"],
