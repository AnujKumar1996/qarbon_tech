...
from src.workorder_operations.subscribe_workorder_notifications import subscribe_workorder_notifications
from src.workorder_operations.unsubscribing_workorder_notifications import unsubscribing_workorder_notifications
from src.workorder_operations.retrive_workorder_subscription import retrive_workorder_subscription
...
router = APIRouter( prefix="/v1/MEF/lsoSonata/workorder",tags=["Workorder Events Subscription"])
...
async def create_hub(
    ...
    try:
        return subscribe_workorder_notifications(order, buyerId, sellerId)
...
async def retrieves_hub_by_id(
    ...
    add_headers(response)
    try:
        return retrive_workorder_subscription(id, buyerId, sellerId)
    except Exception as err:
        return raise_exception(status_msg_code=500,message= str(err), reason="The server encountered an unexpected condition that prevented it from fulfilling the request", reference_error=None, message_code="internalError", property_path=None) 
...
async def unregister_listener(
    ...
    """
    try:
        return unsubscribing_workorder_notifications(id, buyerId, sellerId)
    except Exception as err:
        return raise_exception(status_msg_code=500,message= str(err), reason="The server encountered an unexpected condition that prevented it from fulfilling the request", reference_error=None, message_code="internalError", property_path=None) 
