...
router = APIRouter(
    prefix="/v1/MEF/lsoSonata/productOrderingManagement"
    )
...
async def creates_a_modifyproductorderitemrequesteddeliverydate(
    ...
    try:
        ...
        if request_order_id is None or request_order_id == "":
            ...
            property_path = "productOrder.productOrderId"
            return raise_exception(status_msg_code, message, reason, reference_error, message_code, property_path)
        if request_item_id is None or request_item_id == "":
            ...
            reference_error = None
            message_code = "invalidValue"
            property_path = "productOrderItem.productOrderItemId"
            return raise_exception(status_msg_code, message, reason, reference_error, message_code, property_path)
        else:
            ...
            for order_id, product_data in json_data.items():
                ...
                if request_order_id == product_data["id"]:
                    ...
                    if buyerId != "" and product_data.get("buyerId") != buyerId:
                        status_msg_code = 422
                    ...
                    if sellerId !="" and product_data.get("sellerId") != sellerId:
                        status_msg_code = 422
                    ...
                    if response_href != "" and response_href != product_data.get("href"):
                        status_msg_code = 422
                        message = f"Invalid productOrderHref '{response_href}'"
                        reason = "Validation error"
                        reference_error = None
                        message_code = "invalidValue"
                        property_path = "productOrderItem.productOrderHref"
                        return raise_exception(status_msg_code, message, reason, reference_error, message_code, property_path)
                        
                        
                    product_order_state=product_data.get("state")  
                    if product_order_state != ""  and product_order_state in ( "acknowledged" , "inProgress"):
                        ...
                        reference_error = None
                        message_code = "invalidValue"
                        property_path = "productOrder.state"
                        return raise_exception(status_msg_code, message, reason, reference_error, message_code, property_path)
            ...
            if 'productOrderItem' in order_data:
                ...
                for product_order_item in order_data['productOrderItem']:
                    ...
                    if request_item_id == product_order_item.get('id'):
                        ...
                        if product_order_item_state != "" and  product_order_item_state in ("acknowledged" , "inProgress"):
                            is_item_found = True
                        else:
                            status_msg_code = 422
                            ...
                            reference_error = None
                            message_code = "invalidValue"
                            property_path = "productOrderItem.state"
                            return raise_exception(status_msg_code, message, reason, reference_error, message_code, property_path)
                        ...
                        if requested_Completion_Date != "" and requested_Completion_Date < order_data.get("orderDate"): 
                            status_msg_code = 422
                            message = "The 'requestedCompletionDate' must be greater than 'orderDate' and must be in date-time format."
                            reason = "Validation error"
                            reference_error = None
                            message_code = "invalidValue"
                            property_path = "requestedCompletionDate"
                            return raise_exception(status_msg_code, message, reason, reference_error, message_code, property_path)
                ...
                if not is_item_found:
                    ...
                    reference_error = None
                    message_code = "invalidValue"
                    property_path = "productOrder.productOrderItemId"
                    return raise_exception(status_msg_code, message, reason, reference_error, message_code, property_path)
                    ...
                    if validation is True:
                        ...
                        reference_error = None
                        message_code = "invalidValue"
                        property_path = "modifyProductOrderItemRequestedDeliveryDate"
                        return raise_exception(status_msg_code, message, reason, reference_error, message_code, property_path)
    ...
    try:
        if id:
            ...
            if id in data:
                ...
                if buyerId != "" and order_info.get("buyerId") != buyerId:
                    status_msg_code = 404
                    message = f"Invalid buyerId '{buyerId}'"
                ...
                if sellerId != ""  and order_info.get("sellerId") != sellerId:
                    status_msg_code = 404
                    message = f"Invalid sellerId '{sellerId}'"
...
async def lists_or_finds_modifyproductOrderitemrequesteddeliverydate_objects(
    response: Response, 
    productOrderId: Optional[str]=Query(""),
    ...
    try:
        ...
        for _, order_info in data.items():
            ...
            if((state == "" or order_info.get("state") == state) and
                ...
                (sellerId == "" or order_info.get("sellerId") == sellerId) and 
                (productOrderId == "" or product_order_id == productOrderId)
                ):
