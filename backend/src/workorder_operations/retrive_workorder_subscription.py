...
def retrive_workorder_subscription(id, buyerId, sellerId):
    ...
    try:
        ...
        if id in all_keys and json_data.get(id).get('id') == id:
            ...
            if buyerId != "" and  not check_buyer_id(json_result, buyerId): 
                    return raise_exception(status_msg_code=404, message=f"buyerId not found '{buyerId}'", reason="buyerId not found", reference_error=None, message_code= "notFound", property_path=None)
            if sellerId != "" and not check_seller_id(json_result, sellerId):
                    return raise_exception(status_msg_code=404, message= f"sellerId not Found '{sellerId}'", reason="sellerId not Found", reference_error=None, message_code="notFound", property_path=None)
            ...
            return raise_exception(status_msg_code=404, message=f"Id not found '{id}'",reason= "Id not found", reference_error=None, message_code="notFound", property_path=None)
