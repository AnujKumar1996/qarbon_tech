...
def workorder_appointment_required_event(info):

    try:
        cwd = Path(__file__).parents[1]
        workorder_response_file_name_name="workorder.json"
        workorder_response_file = cwd / "responses" / workorder_response_file_name_name 
        
        if not workorder_response_file.exists() :
            status_msg_code = 404
            message = f"File not found '{workorder_response_file_name_name}'"
        ...
        if info.eventType != "workOrderAppointmentRequiredEvent":
            status_msg_code = 422
            message = "The eventType must be 'workOrderAppointmentRequiredEvent'"
        ...
        if info.event.id not in list_of_keys :
            ...
            appointment_Required = workorder_data.get("appointmentRequired")
            ...
            if appointment_Required:
                ...
                status_msg_code = 422
                message = "The appointment has not been scheduled"
