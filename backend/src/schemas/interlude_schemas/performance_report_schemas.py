...
class FileTransferData(BaseModel):
    ...
    fileFormat: Optional[str] = Field(
        default="", description="Format of the file containing collected data."
    )
    fileLocation: Optional[HttpUrl] = Field(
        default="", description="Location of the file containing collected data."
    )
    transportProtocol: Optional[str] = Field(
        default="", description="Transport protocol to use for file transfer."
    )
    compressionType: Optional[CompressionType] = Field(
        default="", description="Compression types used for the collected data file."
    )
    packingType: Optional[str] = Field(
        default="", description="Specify if the data file is to be packed."
    )
    retentionPeriod: Optional[str] = Field(
        default="", description="A time interval to retain the file."
    )
...
class PerformanceJobValue(PerformanceJobRefOrValue):
    ...
    producingApplicationId : Optional[str] = Field(description="")
...
class PerformanceReport(PerformanceReport_Common):
    ...
    failureReason :  Optional[str] = Field(default="",description='Reason in case report generation failed.')
