...
class MEFProductOrderRef(BaseModel):
    ...
    '''
    productOrderHref: Optional[str] = Field(default="", description="Hyperlink to access the order")
...
class EventSubscriptionInput(BaseModel):
    ...
    """
    callback: str = Field(description = """This callback value must be set to host property from Buyer Product Order Notification API (productOrderNotification.api.yaml). This property is appended with the base path and notification resource path specified in that API to construct an URL to which notification is sent\
    ...
    query:Optional[str] = Field(default = "", description = """This attribute is used to define to which type of events to register  to. Example: "query":"eventType = productOrderStateChangeEvent". To subscribe for more than one event type, put the values separated by comma:\
...
class EventSubscription(BaseModel):
    ...
    """
    callback: str = Field(description = "The value provided by the Buyer in `EventSubscriptionInput` during notification registration",
                          example="https://example.com",
                          pattern=r'^https?://\S+$')
    id:str = Field(description = "An identifier of this Event Subscription assigned by the Seller when a resource is created.")
    query:Optional[str] = Field(default = "", description = "The value provided by the Buyer in `EventSubscriptionInput` during notification registration")
 
 
class CancelProductOrderEventType (str, Enum):
    '''
    Indicates the type of Cancel Product Order event.
    '''
    CANCEL_PRODUCT_ORDER_STATE_CHANGE_EVENT = 'cancelProductOrderStateChangeEvent'
    
class CancelProductOrderEventPayload(BaseModel):
    '''
    The identifier of the Cancel Product Order being subject of this event.
    '''
    sellerId: Optional[str]=Field(default="", description="The unique identifier of the organization that is acting as the Seller.\
        MUST be specified in the request only when requester entity represents more than one Seller. Reference: MEF 79 (Sn 8.8)")
    id: str = Field(description="ID of the Cancel Product Order")
    href: Optional[str] = Field(default="", description="Hyperlink to access the Cancel Product Order")
    buyerId: Optional[str] = Field(default="", description= " The unique identifier of the organization that is acting as the a Buyer. MUST be specified in the request only when the responding represents more than one Buyer. Reference: MEF 79 (Sn 8.8)")
    
    
class CancelProductOrderEvent(BaseModel):
    '''
    Event class is used to describe information structure used for notification.
    ''' 
    ...
    event: CancelProductOrderEventPayload
