...
class Money(BaseModel):
    ...
    """ 
    unit: Optional[str] = Field(default="", description = "Currency (ISO4217 norm uses 3 letters to define the currency)")   
...
class MEFModifyProductOrderItemRequestedDeliveryDateRef(BaseModel):
    ...
    """
    href: Optional[str] = Field(default="", description = "Hyperlink to access the Modify Product Order Item Requested Delivery Date")
...
class MEFProductOrderChargeItem(BaseModel):
    ...
    """
    acceptanceIndicator: Optional[MEFAcceptedRejectedType] = Field(default="",description = "Indicator of acceptance")
    ...
    recurringChargePeriod:Optional[MEFChargePeriod] = Field(default="", description = "Used for a recurring charge to indicate period")
    state: MEFProductOrderChargeItemStateType = Field(description = "Possible values for the state of the Charge Item")
    unitOfMeasure: Optional[str] = Field(default="", description = "Unit of Measure if price depending on it is usageBased (Gb, SMS volume, etc..)")
...
class MEFProductOrderCharge(BaseModel):
    ...
    href: Optional[str] = Field(default="", description = "Hyperlink to the Charge. Hyperlink MAY be used by the Seller in responses Hyperlink MUST be ignored by the Seller in case it is provided by the Buyer in a request")
