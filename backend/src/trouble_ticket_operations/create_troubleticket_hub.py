...
def subscribe_trouble_ticket_notifications(order, buyerId, sellerId):
    
    try:
        order_data = order.model_dump(by_alias=True)
        current_directory = Path(__file__).parents[1]
        response_file = "trouble_ticket_hub.json"
        ...
        if query_param == "":
            ...
            if "eventType=" not in query_param:
                return raise_exception(status_msg_code=400, message="'eventType' is missing in the query value", reason="eventType is missing", reference_error=None, message_code="missingQueryParameter", property_path=None)

            if "&" in query_param and "," in query_param:
                return raise_exception(status_msg_code=400, message=f"Invalid query value '{query_param}'", reason="Request query not a valid value", reference_error=None, message_code="invalidBody", property_path=None)
            
            user_queries_list=re.split("eventType=|&eventType=|,",query_param)
            first_match= user_queries_list[0]
            if first_match !="":
                return raise_exception(status_msg_code=400, message=f"Invalid query value '{first_match}'", reason="Request query not a valid value", reference_error=None, message_code="invalidBody", property_path=None)
                
            user_queries_list.pop(0)
            for query in set(user_queries_list):
                if query not in allowed_values:
                    return raise_exception(status_msg_code=400, message=f"Invalid query value '{query}'", reason="Request query not a valid value", reference_error=None, message_code="invalidBody", property_path=None) 
