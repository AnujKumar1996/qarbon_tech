...
def productspecification_status_change_event_notification(info):
    ...
    try:
        ...
        if info.eventType != "productSpecificationStateChangeEvent":
            status_msg_code = 422
            message = "The eventType must be 'productSpecificationStateChangeEvent'"
        ...
        if info.event.id not in list_of_keys :
            ...
            if current_state != previous_state:
                ...
                create_response_json(info.event.id, category_data, product_specification_response_filename)
                
                return Response(status_code=status.HTTP_204_NO_CONTENT,media_type="application/json;charset=utf-8")
                
            else:
                status_msg_code = 422
                message = "The 'lifecycleStatus' remains unchanged"
                reason = "Validation error"
                reference_error = None
