...
def product_offering_qualification_state_change_Event_notification(info):
    ...
    try:
        ...
        if info.eventType != "poqStateChangeEvent":
            status_msg_code = 422
            message = "The eventType must be 'poqStateChangeEvent'"
        ...
        if info.event.id not in list_of_keys :
            ...
            current_state = poq_data.get("state")
            previous_state = poq_data.get("previoustate")
            
            if current_state != previous_state:
                poq_data["previoustate"] = current_state
