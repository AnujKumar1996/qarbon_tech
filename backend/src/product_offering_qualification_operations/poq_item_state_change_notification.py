...
def poq_item_state_change(info):
    ...
    try:
        ...
        if info.eventType != "poqItemStateChangeEvent":
            status_msg_code = 422
            message = "The eventType must be 'poqItemStateChangeEvent'"
        ...
        if info.event.id not in list_of_keys :
            ...
            product_offering_items = poq_data.get("productOfferingQualificationItem", [])
            result=False
            if info.event.poqItemId != "":
                for product_index,item in enumerate(product_offering_items):
                    if info.event.poqItemId == item.get("id"):
                ...
                status_msg_code = 422
                message = "'event.poqItemId' must be provided"
                reason = "Validation error"
                reference_error = None
                message_code = "invalidValue"
            ...
            if current_state != previous_state:
                poq_data["productOfferingQualificationItem"][index]["previoustate"] = current_state
                create_response_json(info.event.id, poq_data, poq_response_filename)
